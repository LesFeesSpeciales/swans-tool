...
#                traceback.print_exc()
            ...
            if validated:
                try:
                    child.validated()
                except PathItem.PathValueError:
                    err = sys.exc_info()[1]
#                    print '###############', err
#                    import traceback
#                    traceback.print_exc()
    ...
    def __call__(self, **config):
        ...
        if debug:
            print('###-- consumed keys: %s'% consumed_keys)
            print('###-- config keys were:%s'%  config.keys())
